...
EXECUTE_PROMPTS = [
    ...
    (
        "Extrude the selected face up 1 meter",
        "EXECUTE",
        r"extrude|bpy\.ops\.mesh\.extrude",
    ),
...
def eval_blender_exec(
    model: ModelClient, blender_path: str | None, n: int = 500
) -> dict[str, Any]:
...
def eval_task_type_accuracy(model: ModelClient) -> dict[str, Any]:
    ...
    for voice, task_type, keywords in PHYSICS_REASONING_PROMPTS:
        ...
        if is_correct:
            correct += 1
            breakdown["UNDERSTAND"]["correct"] = (
                breakdown["UNDERSTAND"].get("correct", 0) + 1
            )
...
def eval_multi_turn_coherence(model: ModelClient, n: int = 50) -> dict[str, Any]:
    ...
    for scenario in scenarios:
        ...
        for turn in turns:
            ...
            response, _ = model.generate(
                full_prompt, software="blender", max_tokens=384
            )
        ...
        ctx_matches = sum(1 for ctx in check_ctxs if ctx.lower() in all_responses_text)
    ...
    print(
        f"  Coherence rate: {coherence * 100:.1f}% ({total_coherent}/{len(scenarios)})"
    )
...
def eval_latency(model: ModelClient, n: int = 100) -> dict[str, Any]:
    ...
    for voice, _, _ in bar:
        _, lat = model.generate(
            voice, software="blender", max_tokens=128, temperature=0.0
        )
...
def write_report(
    ...
    if compare_path and compare_results:
        ...
        for label, key_path in metrics_to_compare:
            ...
            for k in keys:
                baseline_v = (
                    baseline_v.get(k, {}) if isinstance(baseline_v, dict) else "N/A"
                )
                current_v = (
                    current_v.get(k, {}) if isinstance(current_v, dict) else "N/A"
                )
    ...
    lines.append(
        f"- **Overall:** {tt.get('accuracy', 0) * 100:.1f}%  ({tt.get('correct', '?')}/{tt.get('total', '?')})"
    )
    lines.append("")
    ...
    for k, v in (tt.get("by_type") or {}).items():
        lines.append(
            f"| {k} | {v['accuracy'] * 100:.1f}% | {v['correct']} | {v['total']} |"
        )
    ...
    for sw, v in (cs.get("by_software") or {}).items():
        lines.append(
            f"| {sw} | {v['accuracy'] * 100:.1f}% | {v['correct']} | {v['total']} |"
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--compare", type=str, default=None, help="Baseline model to compare against"
    )
    parser.add_argument(
        "--quick", action="store_true", help="Fast eval: 100 samples, skip Blender exec"
    )
    parser.add_argument(
        "--no-vllm", action="store_true", help="Use HuggingFace instead of vLLM"
    )
    parser.add_argument("--blender-path", type=str, help="Path to Blender executable")
    parser.add_argument(
        "--exec-n", type=int, default=500, help="Blender exec eval sample count"
    )
