...
FROM_STEP=1; SKIP_TRAINING=false; SKIP_DISCOVERY=false; DRY_RUN="${DRY_RUN:-0}"
for arg in "$@"; do
    case "$arg" in
...
STEP=0; TIMINGS=(); START=$(date +%s)

step() {
    STEP=$((STEP+1))
    [ "$STEP" -lt "$FROM_STEP" ] && { echo -e "  ${YELLOW}[SKIP]${NC} Step $STEP: $1"; return 0; }
    echo -e "\n${CYAN}━━━ Step $STEP: $1 ━━━${NC}"; STEP_START=$(date +%s); return 1
}
run() {
    echo -e "  ${CYAN}>>>${NC} $*"
    [ "$DRY_RUN" = "1" ] && return 0
...
if ! step "Environment validation"; then
    run "bash scripts/check_env.sh"; done_step; fi
...
if ! step "Bulk transcript fetch (30 workers)"; then
    run "python3 discovery/fetch_bulk.py --workers 30"; done_step; fi

# ── Step 6: Objaverse prep ────────────────────────────────────────────────────
if ! step "Objaverse 3D dataset prep (50k objects)"; then
    run "python3 discovery/objaverse_prep.py --limit 50000"; done_step; fi
...
if ! step "Collect cross-software and integration data"; then
    run "python3 integrations/collect_design_physics.py --all"; done_step; fi
...
if ! step "Quality validate + dedup all synthesized data"; then
    run "python3 validation/validate.py"; done_step; fi

# ── Step 13: Blender execution validation ────────────────────────────────────
if ! step "Blender headless execution validation (6 workers)"; then
    run "python3 validation/validate_blender.py --workers 6"; done_step; fi
...
if ! step "Training data prep — all streams, quality weights, curriculum order"; then
    run "python3 training/train_prep.py"; done_step; fi
...
if ! step "Health check + smoke test"; then
    run "python3 scripts/health_check.py"; done_step; fi
