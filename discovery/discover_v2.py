...
SEARCH_QUERIES = BLENDER_SEARCH_QUERIES + CROSS_SOFTWARE_QUERIES + DESIGN_PHYSICS_QUERIES
...
def filter_by_quality(video_ids: list[str], api_key: str, min_views: int = 5000) -> list[str]:
    """
...
def main():
    parser = argparse.ArgumentParser(description="Discover 3D tutorial video IDs at scale across all major software")
    parser.add_argument("--api-key", default=os.environ.get("YOUTUBE_API_KEY", ""))
    parser.add_argument("--channels", action="store_true", help="Crawl all known 3D channels")
    parser.add_argument("--search", action="store_true", help="Run Blender keyword search queries")
    parser.add_argument("--all-software", action="store_true",
    ...
    parser.add_argument("--design-physics", action="store_true",
    ...
    parser.add_argument("--min-views", type=int, default=5000, help="Min view count when --filter-quality is set")
    parser.add_argument("--search-pages", type=int, default=1, help="Pages per search query (100 units each)")
    ...
    if not (args.channels or args.search or args.all_software or args.design_physics):
        print("Specify at least one mode: --channels, --search, --all-software, --all-categories, or --design-physics")
        return
    ...
    if args.channels:
        ...
        if extra_channels:
            ...
            for ch_url, sw in extra_channels.items():
                ...
                print(f"    [info] {ch_name} ({sw}) — add channel ID to ALL_CHANNELS to crawl")
    ...
    if args.all_software:
        ...
        print(f"  Maya, Cinema 4D, Houdini, ZBrush, Substance, Rhino, Unreal, Fusion 360, 3ds Max, SketchUp")
        print(f"Quota cost: ~{len(CROSS_SOFTWARE_QUERIES) * args.search_pages * 100} units\n")
    ...
    if args.design_physics:
        ...
        print(f"  PBR, topology, physics simulations, design disciplines")
        print(f"Quota cost: ~{len(DESIGN_PHYSICS_QUERIES) * args.search_pages * 100} units\n")
