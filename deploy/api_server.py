...
class CommandRequest(BaseModel):
    ...
    scene_context: dict[str, Any] = Field(
        default_factory=dict, description="Current scene state from the DCC plugin"
    )
    software: str = Field(
        default="blender",
    ...
    session_id: str | None = Field(
        default=None, description="Session ID for multi-turn context"
    )
...
class QARequest(BaseModel):
    scene_context: dict[str, Any] = Field(
        default_factory=dict, description="Scene state from the DCC plugin"
    )
    blender_python: str | None = Field(
        default=None,
    ...
    auto_fix: bool = Field(
        default=False, description="If true, include fix_command code in the response"
    )
...
async def init_vllm() -> None:
    ...
    logger.info(
        "Initializing vLLM engine", extra={"model": MODEL_PATH, "tp_size": TP_SIZE}
    )
    try:
...
def build_messages(
    voice: str, software: str, scene_context: dict, history: list[dict]
) -> list[dict]:
...
async def generate_vllm(
    messages: list[dict], max_tokens: int, temperature: float, request_id: str
) -> tuple[str, float]:
    ...
    async for output in state.engine.generate(
        text_input, params, request_id=request_id
    ):
        if output.outputs:
...
async def command(
    ...
    if not parsed.get("blender_python") and not parsed.get("universal_dsl"):
        logger.warning(
            "JSON parse failed on first attempt — retrying with stricter prompt",
        ...
        raw2, latency2 = await generate(
            retry_messages, req.max_tokens, 0.05, request_id + "-retry"
        )
    ...
    primary_code = code_by_software.get(
        f"{req.software}_python", parsed.get("blender_python", "")
    )
...
async def plan(
    ...
    try:
        ...
        steps_data = [
            {
                "step": 1,
                "voice_command": req.intent,
                "blender_python": raw,
                "reasoning": "Parsed as single step",
            }
        ]
...
async def materialize(
    ...
    messages = [
        ...
        {
            "role": "user",
            "content": f"[{req.software.upper()}] Create a PBR material: {req.material_name}",
        },
    ]
...
async def qa(
    ...
    for obj in objects:
        ...
        if obj.get("has_unapplied_transform"):
            ...
            scale_bad = isinstance(scale, list) and any(
    ...
    if render_engine == "BLENDER_EEVEE":
        info.append(
            "Using EEVEE — switch to Cycles for physically accurate renders and bakes."
        )
    ...
    if req.blender_python:
                ...
                warnings.append(
                    {
                        ...
                        "fix_command": snippet.replace(
                            dep, "bpy.ops.mesh.merge_by_distance"
                        ),
                        "auto_fixable": True,
                    }
                )
        ...
        if re.search(r'bpy\.data\.objects\["[^"]+"\]', snippet):
            info.append(
                "Code uses hardcoded object names — may fail if scene naming changes."
            )
