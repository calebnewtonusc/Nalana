...
def pack_conversation(
    messages: list[dict], task_type: str, source: str, quality: float = 3.0
) -> dict:
    return {
...
def chain_pairs_to_conversation(pairs: list[dict], max_turns: int = 20) -> dict:
    ...
    task_type = (
        "MATERIALIZE"
        if has_material and not has_build
        else "BUILD"
        if has_build
        else "EXECUTE"
    )
...
def _json_step(
    blender_python: str, op: str, reasoning: str, args: dict | None = None
) -> str:
    payload = {
...
def generate_synthetic_conversation(template: dict, variation_seed: int = 0) -> dict:
    ...
    messages.append(
        make_message(
            "assistant", _json_step(first_python, "MULTI_STEP_PLAN", first_reasoning)
        )
    )
    ...
    for i, (voice, python, reasoning) in enumerate(steps[1:], start=1):
        ...
        messages.append(
            make_message("assistant", _json_step(python, f"STEP_{i}", reasoning))
        )
...
def generate_synthetic(template_filter: str, count: int) -> list[dict]:
    ...
    print(
        f"[synthetic] Generated {len(conversations)} synthetic conversations "
...
def main() -> None:
    ...
    parser.add_argument(
        "--templates", default="all", help="Comma-separated template names, or 'all'"
    )
    parser.add_argument(
        "--count",
        type=int,
